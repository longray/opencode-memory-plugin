...
export const memory_write = tool({
  description:
    "Write an entry to long-term memory with optional Atom tree structure.",
  args: {
    ...
    atoms: tool.schema
      ...
      .optional()
      .describe("Optional Atom tree structure for hierarchical knowledge"),
  },
  async execute(args) {
...
export const memory_search = tool({
  description:
    "Search memory with configurable search mode and optional Atom scope",
  args: {
...
#### ✅ 缺口 4：Agent 无法自动萃取 Atom — PARTIALLY_RESOLVED
...
**状态**: PARTIALLY_RESOLVED — Prompt 注入完成，Agent 实际行为待观察验证（验收标准见 Phase 2 验证方案）。
...
| 2.4 验证 Agent 行为                   | OpenCode 实际使用  | ✅ COMPLETED (Agent 行为验证 ⏳) |
...
1. 在 `code-analysis-service.js` 中实现自动 Atom 化后调用 `linkToConversationMemory()`
...
- [集成实施手册](./IMPLEMENTATION-INTEGRATION.md)
- [测试计划](./test-plans/)（unit-test-plan.md / integration-test-plan.md / e2e-test-plan.md）
...
| `tools/core.js`                | memory*write / entity_update / entity_atoms / load_context*\* |
