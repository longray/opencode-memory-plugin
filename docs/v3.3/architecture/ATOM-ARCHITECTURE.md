...
abstract: "Vue 3 最佳实践" # L0: 摘要，建议 ≤100 字符（超长会警告但不拒绝）
overview: # L1: 结构化概览
  chapters: 3
...
- [x] **统一搜索端点** ✅ COMPLETED
  ...
  - 支持跨 Entity + Atom 搜索（`scope: all|entity|atom`）

### Phase 2: 插件端核心（1 周）

- [x] **存储层** ✅ COMPLETED
  - 修改 `buildEntryContent` 支持 Atoms 段（`entry.js:14-23`）
  - 修改 `extractSections` 解析 Atoms JSON（`extractor.js:41-61`）
  - 实现 `buildAtomTree` O(n) 算法（`atom-tree.js:13`）
  - 实现 `flattenAtomTree` 扁平化（`atom-tree.js:63`）
- [x] **API 层** ✅ COMPLETED
...
- [x] **风险缓解** ✅ COMPLETED
  - 循环引用检测（三色 DFS）（`atom-tree.js:88`）
  - 悬挂引用处理（`atom-tree.js:252`）
  - 死链标记（`memory-core.js:markDeadLinks`）
- [ ] **Obsidian 导入/导出** ⏳ NOT IMPLEMENTED
  - 新增 `import_obsidian` 工具
  - 新增 `export_obsidian` 工具

### Phase 4: 测试与优化（1 周）

- [x] **单元测试** ✅ COMPLETED（14 个测试文件，`tests/unit/atoms/`）
- [x] **集成测试** ✅ COMPLETED（`tests/integration/v3.3-atom-e2e.test.js`）
- [ ] **性能测试** ⏳ IN PROGRESS（大文件 > 100 Atom 基准测试尚未完成）
- [x] 文档更新 ✅ COMPLETED（AGENTS.md、README.md 已更新 Atom 相关内容）
