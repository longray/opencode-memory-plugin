...
const ENTITY2_ATOMS = [
  {
    ...
    name: "第1章：setup 函数",
    content:
      "setup() 是 Vue 3 Composition API 的入口函数。它在组件创建之前执行，此时 props 已经被解析，是访问响应式数据、计算属性、生命周期钩子等的起点。setup 接收 props 和 context 两个参数，返回的对象属性将暴露给模板。在 Vue 3.3+ 中，`<script setup>` 语法糖使 setup 的使用更加简洁，无需显式 return。详见 [[EVAL2S01]] 了解 setup 返回值与模板绑定的机制。",
    order: "a0",
    heading_level: 1,
    parent_id: null,
    children: [
  ...
  },
  {
    ...
    content:
      "Vue 3 提供了 ref 和 reactive 两种创建响应式数据的方式。ref 适用于基础类型值（string、number、boolean），通过 .value 访问；reactive 适用于对象类型，直接访问属性即可。选择 ref 还是 reactive 取决于数据结构和使用场景，ref 更灵活且在 `<script setup>` 中使用更简洁。详见 [[EVAL2S02]] 了解 ref 的解包机制。",
    order: "a1",
    heading_level: 1,
    parent_id: null,
    children: [
  ...
  },
  {
    ...
    content:
      "computed 用于定义基于其他响应式数据的派生值，具有缓存特性——只有依赖变化时才会重新计算。watch 和 watchEffect 用于观察响应式数据变化并执行副作用。computed 返回只读的 ref，watch 更灵活（可配置 immediate、deep、flush），watchEffect 自动追踪依赖。详见 [[EVAL2S03]] 了解 watchEffect 的自动依赖收集。",
    order: "a2",
    heading_level: 1,
    parent_id: null,
    children: [
  ...
  },
  {
    ...
    name: "第4章：生命周期钩子",
    content:
      "Composition API 通过 on 前缀的函数来注册生命周期钩子：onMounted、onUpdated、onUnmounted、onBeforeMount 等。这些函数在 setup 中调用，将回调注册到对应的生命周期阶段。与 Options API 的 beforeCreate 和 created 对应的逻辑直接写在 setup 中即可。详见 [[EVAL2S04]] 了解 onMounted 的典型使用场景。",
    order: "a3",
    heading_level: 1,
    parent_id: null,
    children: [
...
const ENTITY3_ATOMS = [
  {
    ...
    name: "第1章：Readable 流",
    content:
      "Readable 流是数据的来源抽象，支持两种消费模式：流动模式（flowing）和暂停模式（paused）。流动模式下数据自动从底层系统读取并通过事件发射；暂停模式下需要显式调用 read() 方法。Node.js 中的 fs.createReadStream、HTTP IncomingMessage、process.stdin 都是 Readable 流的实现。Readable 流是理解 Node.js 流式处理的基础，详见 [[EVAL3S01]] 了解 pipe 方法如何简化流的连接。",
    order: "a0",
    heading_level: 1,
    parent_id: null,
    children: [],
  },
  {
    ...
    name: "第2章：Writable 流",
    content:
      "Writable 流是数据的目的地抽象，提供了 write()、end()、finish 等方法。写入操作内部维护了一个缓冲区队列，当 write() 返回 false 时表示缓冲区已满，此时应暂停读取。Writable 流常用于文件写入（fs.createWriteStream）、HTTP 响应、TCP socket、进程标准输出等场景。详见 [[EVAL3S02]] 了解背压处理的核心机制。",
    order: "a1",
    heading_level: 1,
    parent_id: null,
    children: [],
  },
  {
    ...
    content:
      "Transform 流是 Duplex 流的特殊形式，在读取和写入之间对数据进行转换。它同时实现了 Readable 和 Writable 接口，数据从 Writable 端流入，经过 _transform() 方法处理后从 Readable 端流出。常见的 Transform 流包括 zlib（压缩/解压）、crypto（加密/解密）、StringDecoder（编码转换）以及自定义数据处理管道。详见 [[EVAL3S03]] 了解流式处理的错误处理策略。",
    order: "a2",
    heading_level: 1,
    parent_id: null,
    children: [],
  ...
  },
  {
    ...
    name: "3.2 背压处理机制",
    content:
      "背压是流式处理的核心概念：当消费者处理速度跟不上生产者时，通过信号机制让生产者暂停，避免内存溢出。Writable 流的 write() 返回 false 表示内部缓冲区已满（默认 highWaterMark 为 16KB），触发 'drain' 事件后可以继续写入。详见 [[EVAL3CH02]] 了解 Writable 流的缓冲区机制。\n\n```javascript\nfunction write(stream, data) {\n  if (!stream.write(data)) {\n    return new Promise(resolve => stream.once('drain', resolve));\n  }\n  return Promise.resolve();\n}\n\nasync function writeAll(stream, items) {\n  for (const item of items) {\n    await write(stream, JSON.stringify(item) + '\\n');\n  }\n  stream.end();\n}\n```",
    order: "a1",
    heading_level: 2,
    parent_id: "EVAL3CH02",
    children: [],
  },
  {
    ...
    name: "3.3 流式错误处理",
    content:
      "流式处理中的错误传播与普通 try/catch 不同。如果 Readable 流发生错误，通过 pipe 连接的下游 Writable 流不会自动收到通知。推荐使用 pipeline() 替代 pipe()，它会自动传播错误并正确清理所有流资源。详见 [[EVAL3CH03]] 了解 Transform 流的错误处理。\n\n```javascript\nconst { pipeline } = require('stream/promises');\n\ntry {\n  await pipeline(\n    fs.createReadStream('input.txt'),\n    zlib.createGzip(),\n    fs.createWriteStream('output.gz')\n  );\n  console.log('管道完成');\n} catch (err) {\n  console.error('管道失败:', err);\n}\n```\n\npipeline 自动处理：错误传播、流销毁、文件描述符关闭。",
    order: "a2",
    heading_level: 2,
    parent_id: "EVAL3CH03",
    children: [],
...
const ENTITY4_ATOMS = [
  {
    ...
    name: "第1章：GitFlow",
    content:
      "GitFlow 是最经典的分支管理模型，由 Vincent Driessen 于 2010 年提出。它定义了 master、develop、feature、release、hotfix 五种分支类型，每个分支有明确的职责和生命周期。GitFlow 适合发布周期较长的项目（如企业级 SaaS），能清晰区分开发、测试和生产环境。缺点是分支数量多、合并流程复杂，不适合持续部署。详见 [[EVAL4S01]] 了解 feature 分支的具体使用规范。",
    order: "a0",
    heading_level: 1,
    parent_id: null,
    children: [],
  },
  {
    ...
    content:
      "GitHub Flow 是 GitHub 推荐的简化分支模型，只有 main 分支和 feature 分支两种。开发流程：从 main 拉分支 → 开发 → 创建 Pull Request → Code Review → 合并到 main → 立即部署。GitHub Flow 适合持续部署的 Web 应用，强调快速迭代和 Code Review。相比 GitFlow，它大幅减少了分支管理的复杂度。详见 [[EVAL4S02]] 了解 release 分支在 GitHub Flow 中的替代方案。",
    order: "a1",
    heading_level: 1,
    parent_id: null,
    children: [],
  },
  {
    ...
    content:
      "Trunk-Based Development（主干开发）是 Google、Facebook 等大型互联网公司采用的模式。所有开发者直接在 main（trunk）上提交，通过 Feature Flag 控制未完成功能的可见性。短命分支（不超过一天）用于 Code Review。Trunk-Based 强调频繁集成（每天多次提交到主干）、自动化测试（CI/CD 全覆盖）和渐进式发布。详见 [[EVAL4S03]] 了解 hotfix 在 Trunk-Based 模式下的处理方式。",
    order: "a2",
    heading_level: 1,
    parent_id: null,
    children: [],
  },
  {
    ...
    content:
      "feature 分支从 develop 分支创建，用于开发新功能。命名约定：feature/用户故事编号-简短描述（如 feature/PROJ-123-user-auth）。开发完成后通过 Pull Request/Merge Request 合并回 develop 分支。feature 分支的生命周期应尽量短（一般不超过 2 周），避免长期分支导致的合并冲突。详见 [[EVAL4CH01]] 了解 GitFlow 的完整分支体系。\n\n```\ngit checkout develop\ngit checkout -b feature/PROJ-123-user-auth\n# ... 开发 ...\ngit checkout develop\ngit merge --no-ff feature/PROJ-123-user-auth\n```",
    order: "a0",
    heading_level: 2,
    parent_id: "EVAL4CH01",
    children: [],
  },
  {
    ...
    content:
      "release 分支用于准备新版本发布，从 develop 分支创建。在 release 分支上只允许 bug 修复、版本号更新和文档更新，不允许添加新功能。测试通过后同时合并到 master（打 tag）和 develop。GitHub Flow 不需要独立的 release 分支，通过 GitHub Releases 和 Tag 管理发布。详见 [[EVAL4CH02]] 了解 GitHub Flow 的简化发布流程。\n\n```\n# GitFlow release 流程\ngit checkout -b release/1.2.0 develop\n# 修复 bug、更新版本号\ngit checkout master && git merge --no-ff release/1.2.0\ngit tag -a v1.2.0\ngit checkout develop && git merge --no-ff release/1.2.0\n```",
    order: "a1",
    heading_level: 2,
    parent_id: "EVAL4CH02",
    children: [],
  },
  {
    ...
    content:
      "hotfix 分支用于修复生产环境的紧急问题，从 master（或 main）直接创建。修复完成后合并回 master 并打新版本 tag，同时合并回 develop 以防止问题再次出现。在 Trunk-Based Development 中，hotfix 通常直接提交到 trunk 并通过 Feature Flag 控制回滚，不需要独立分支。详见 [[EVAL4CH03]] 了解 Trunk-Based 的渐进式发布策略。\n\n```\n# GitFlow hotfix 流程\ngit checkout -b hotfix/1.2.1 master\n# 修复紧急 bug\ngit checkout master && git merge --no-ff hotfix/1.2.1\ngit tag -a v1.2.1\ngit checkout develop && git merge --no-ff hotfix/1.2.1\n```",
    order: "a2",
    heading_level: 2,
    parent_id: "EVAL4CH03",
    children: [],
...
async function main() {
  ...
  for (const seed of SEEDS) {
    ...
    try {
      const response = await client.http.post("/api/v1/entities", {
        type: seed.type,
