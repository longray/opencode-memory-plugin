...
async function evaluateSearchPerformance(queryFile, topK = 5) {
  try {
    ...
    for (const testCase of queryData.queries) {
      ...
      const entityResults = await searchWithScope(testCase.query, 'entity', topK);
      ...
      report.atom_scope.mrr.push(calculateMRR(atomResults.results, relevantIds));
      ...
      report.entity_scope.mrr.push(calculateMRR(entityResults.results, relevantIds));
    ...
    report.summary = {
      atom_scope: {
        avg_precision_at_k: Math.round(avg(report.atom_scope.precision_at_k) * 1000) / 1000,
        avg_mrr: Math.round(avg(report.atom_scope.mrr) * 1000) / 1000,
        avg_response_time_ms: Math.round(avg(report.atom_scope.response_times) * 100) / 100,
      },
      entity_scope: {
        avg_precision_at_k: Math.round(avg(report.entity_scope.precision_at_k) * 1000) / 1000,
        avg_mrr: Math.round(avg(report.entity_scope.mrr) * 1000) / 1000,
        avg_response_time_ms: Math.round(avg(report.entity_scope.response_times) * 100) / 100,
      },
    };
...
if (args.help || !args.queryFile) {
  console.error('Usage: node evaluate-search-performance.js [options] <queryFile>');
  console.error('');
