...
- ✅ **自动触发** — 文件保存后自动分析，无需手动操作
- ✅ **AST 分析** — 使用 Oxc 解析器进行精确的语法树分析
- ✅ **批量上传** — 智能批量处理，减少网络请求
...
| 语言           | 扩展名                        | 分析内容                         |
| -------------- | ----------------------------- | -------------------------------- |
| **JavaScript** | `.js`, `.mjs`, `.cjs`         | 函数、类、导入、导出             |
| **TypeScript** | `.ts`, `.mts`, `.cts`, `.tsx` | 函数、类、接口、类型、导入、导出 |
| **Python**     | `.py`                         | 函数、类、导入                   |
| **Go**         | `.go`                         | 函数、类型、导入                 |
| **Rust**       | `.rs`                         | 函数、结构体、trait、模块        |
| **Java**       | `.java`                       | 类、方法、字段、导入             |
