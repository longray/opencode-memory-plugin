...
function buildQueue() {
  ...
  queue.wrapperClient = {
    ...
    createReference: jest.fn().mockResolvedValue({ id: 'ref:1', type: 'calls' }),
...
describe('Auto-Sync Pipeline: AnalysisQueue', () => {
  ...
  describe('P0: End-to-End Flow', () => {
    it('E2E-1: single file save → analyze → upload atoms/entity/references', async () => {
      ...
      expect(queue.wrapperClient.createAtom).toHaveBeenCalled();
      ...
      expect(queue.wrapperClient.createReference).toHaveBeenCalled();
    });
    ...
    it('E2E-2: multiple files saved → all processed', async () => {
      ...
      expect(queue.wrapperClient.createEntity).toHaveBeenCalledTimes(3);
    });
  ...
  describe('P1: Queue Behavior', () => {
    ...
    it('Q-2: unsupported file type (.md) → skipped', async () => {
      ...
      expect(queue.wrapperClient.createAtom).not.toHaveBeenCalled();
    });
  ...
  describe('P3: Error Recovery', () => {
    ...
    it('ER-2: all createAtom fail → entity not created, no crash', async () => {
      queue.wrapperClient.createAtom.mockRejectedValue(new Error('network error'));
    ...
    it('ER-3: createAtom intermittent failure → entity not created, no crash', async () => {
      queue.wrapperClient.createAtom.mockRejectedValueOnce(new Error('timeout'));
