...
describe('WrapperClient Atom/Entity/Reference API', () => {
  ...
  describe('createAtom Validation', () => {
    test('should reject invalid atom type with WrapperError', async () => {
      let caughtError;
      try {
        await client.createAtom({
          type: 'invalid_type',
      ...
      expect(caughtError).toBeInstanceOf(WrapperError);
      expect(caughtError.statusCode).toBe(400);
      expect(caughtError.retryable).toBe(false);
      expect(caughtError.message).toContain('Invalid atom type');
      expect(caughtError.message).toContain('invalid_type');
      // Should not make any HTTP call
      expect(mockFetch).not.toHaveBeenCalled();
    });

    test('should reject empty atom type with WrapperError', async () => {
      await expect(
        client.createAtom({
          type: '',
          content: 'some content',
    ...
    test('should accept all valid atom types', async () => {
      const validTypes = [
      ...
      // All 10 valid types should have triggered fetch calls
