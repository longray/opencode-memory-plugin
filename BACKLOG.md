...
> 基于 GitNexus 功能分析，实现完整的代码分析功能

### BL-401: 设计文档修复（严重问题）

**真实场景**: 设计文档存在 3 个严重问题：语言数量不一致（13 vs 14）、后端图数据库未确定、缺少与记忆系统集成设计。
...
| **前置依赖** | 无                                                                                                                                                                                           |
| **完成标准** | 1. 语言数量统一为 14 种（与 GitNexus 一致）<br>2. 后端图数据库明确（或标注"待定"）<br>3. 新增"与记忆系统集成"章节                                                                            |
| **验证方式** | 1. `grep -c "13 种" docs/CODE-ANALYSIS-DESIGN.md` 返回 0<br>2. `grep -c "Neo4j?" docs/CODE-ANALYSIS-DESIGN.md` 返回 0<br>3. `grep -c "与记忆系统集成" docs/CODE-ANALYSIS-DESIGN.md` 返回 > 0 |

**修复内容**:

- 第 40 行: "13 种编程语言" → "14 种编程语言"
- 第 446 行: "(Neo4j?)" → "（待定）" 或明确选择
- 新增章节: "与记忆系统集成"，说明代码分析结果如何保存到记忆系统

---

### BL-402: 设计文档补充（高优先级问题）

**真实场景**: 设计文档缺少性能指标、增量索引设计、错误处理设计。
...
| **涉及范围** | `docs/CODE-ANALYSIS-DESIGN.md`                                                                                                                                                                  |
| **前置依赖** | BL-401 完成                                                                                                                                                                                     |
| **完成标准** | 1. 新增"性能指标"章节<br>2. 新增"增量索引器"模块设计<br>3. 新增"错误处理"章节                                                                                                                   |
| **验证方式** | 1. `grep -c "性能指标" docs/CODE-ANALYSIS-DESIGN.md` 返回 > 0<br>2. `grep -c "增量索引" docs/CODE-ANALYSIS-DESIGN.md` 返回 > 0<br>3. `grep -c "错误处理" docs/CODE-ANALYSIS-DESIGN.md` 返回 > 0 |

**补充内容**:

- 性能指标: 1000 文件 < 30s, 搜索 < 100ms, 内存 < 500MB
- 增量索引: 文件变化检测、索引更新策略、删除文件处理
- 错误处理: 解析失败、部分索引失败、同步失败的处理策略

---

### BL-403: 设计文档优化（中优先级问题）

**真实场景**: 设计文档缺少配置设计、测试策略、迁移策略。
...
| **涉及范围** | `docs/CODE-ANALYSIS-DESIGN.md`                                                                                                                                                                  |
| **前置依赖** | BL-402 完成                                                                                                                                                                                     |
| **完成标准** | 1. 新增"配置设计"章节<br>2. 新增"测试策略"章节<br>3. 新增"迁移策略"章节                                                                                                                         |
| **验证方式** | 1. `grep -c "配置设计" docs/CODE-ANALYSIS-DESIGN.md` 返回 > 0<br>2. `grep -c "测试策略" docs/CODE-ANALYSIS-DESIGN.md` 返回 > 0<br>3. `grep -c "迁移策略" docs/CODE-ANALYSIS-DESIGN.md` 返回 > 0 |

**补充内容**:

- 配置设计: codeAnalysis.enabled, languages, excludePatterns, maxFileSize
- 测试策略: 单元测试、集成测试、性能测试
- 迁移策略: 现有数据处理、平滑升级、回滚方案

---

### BL-404: 基础 AST 解析实现（阶段 1）

**真实场景**: 需要实现代码分析功能的基础 - AST 解析。使用 tree-sitter 解析 JavaScript/TypeScript 代码。

**目标**: 实现 CodeParser 类，支持 JS/TS 解析

| 项目         | 内容                                                                                                                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `package.json`（添加 tree-sitter 依赖）<br>2. `lib/code-parser.js`（新增）<br>3. `parsers/javascript.js`（新增）<br>4. `parsers/typescript.js`（新增）<br>5. `tests/test-code-parser.test.js`（新增） |
| **前置依赖** | BL-403 完成（设计文档定稿）                                                                                                                                                                              |
| **完成标准** | 1. tree-sitter 依赖安装成功<br>2. CodeParser 类能解析 JS/TS 文件<br>3. 能提取函数、类、接口等符号<br>4. 测试覆盖率 ≥ 20%                                                                                 |
...
### BL-405: 符号提取实现（阶段 1 续）

**真实场景**: 在 AST 解析基础上，实现详细的符号提取功能。

**目标**: 实现 FeatureExtractor 类，提取函数签名、类结构、接口定义

| 项目         | 内容                                                                                                                   |
| ------------ | ---------------------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `lib/feature-extractor.js`（新增）<br>2. `tests/test-feature-extractor.test.js`（新增）                             |
| **前置依赖** | BL-404 完成                                                                                                            |
| **完成标准** | 1. 能提取函数签名（参数、返回值、类型）<br>2. 能提取类结构（方法、属性、继承）<br>3. 能提取接口定义<br>4. 能提取装饰器 |
| **验证方式** | 1. `npm run lint` 通过<br>2. `npm test` 通过<br>3. 测试覆盖率 ≥ 20%                                                    |
...
**真实场景**: 需要解析代码中的导入、调用、继承关系，构建依赖图。

**目标**: 实现 DependencyResolver 类，解析导入、调用、继承

| 项目         | 内容                                                                                           |
| ------------ | ---------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `lib/dependency-resolver.js`（新增）<br>2. `tests/test-dependency-resolver.test.js`（新增） |
| **前置依赖** | BL-405 完成                                                                                    |
| **完成标准** | 1. 能解析 import 语句<br>2. 能解析函数调用关系<br>3. 能解析类继承关系<br>4. 能计算置信度       |
| **验证方式** | 1. `npm run lint` 通过<br>2. `npm test` 通过<br>3. 测试覆盖率 ≥ 20%                            |
...
**真实场景**: 需要将解析结果存储为知识图谱，支持查询和导出。

**目标**: 实现 CodeGraph 类，构建本地知识图谱

| 项目         | 内容                                                                              |
| ------------ | --------------------------------------------------------------------------------- |
| **涉及范围** | 1. `lib/code-graph.js`（新增）<br>2. `tests/test-code-graph.test.js`（新增）      |
| **前置依赖** | BL-406 完成                                                                       |
| **完成标准** | 1. 能添加节点和边<br>2. 能查询节点和关系<br>3. 能导出为 JSON<br>4. 能从 JSON 导入 |
| **验证方式** | 1. `npm run lint` 通过<br>2. `npm test` 通过<br>3. 测试覆盖率 ≥ 20%               |
...
### BL-408: MCP 工具实现（阶段 8）

**真实场景**: 需要将代码分析功能暴露为 MCP 工具，供 AI 代理使用。

**目标**: 实现 code_search、code_context、code_impact、code_analyze 工具

| 项目         | 内容                                                                                                                                           |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `tools/code-analysis.js`（新增）<br>2. `plugin.js`（修改，注册新工具）<br>3. `tests/test-code-analysis.test.js`（新增）                     |
| **前置依赖** | BL-407 完成                                                                                                                                    |
| **完成标准** | 1. code_search 工具能搜索代码符号<br>2. code_context 工具能获取符号详情<br>3. code_impact 工具能分析影响<br>4. code_analyze 工具能分析代码结构 |
| **验证方式** | 1. `npm run lint` 通过<br>2. `npm test` 通过<br>3. 手动测试各工具功能                                                                          |

**预计时间**: 1 周

---

### BL-409: 后端集成设计（阶段 9 设计）

**真实场景**: 需要设计与后端的集成方案，包括 API 接口和同步策略。

**目标**: 完成后端集成设计文档

| 项目         | 内容                                                                                                       |
| ------------ | ---------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `docs/CODE-ANALYSIS-DESIGN.md`（更新"与后端集成"章节）<br>2. `docs/API-CONTRACT.md`（新增代码分析 API） |
| **前置依赖** | BL-408 完成                                                                                                |
| **完成标准** | 1. 后端 API 接口设计完成<br>2. 同步策略设计完成<br>3. 数据流向图完成                                       |
| **验证方式** | 1. 设计文档包含完整的 API 接口定义<br>2. 设计文档包含同步策略说明<br>3. 设计文档包含数据流向图             |
...
> 将 markdownlint-cli2 集成到质量门禁体系，提升文档质量

### BL-501: Marksman LSP 功能验证 ✅

**真实场景**: marksman 已安装并配置为 LSP，但需要验证其在实际使用中的功能是否正常。

**目标**: 验证 marksman LSP 的各项功能

| 项目         | 内容                                                                                                           |
| ------------ | -------------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `README.md`<br>2. `AGENTS.md`<br>3. `docs/*.md`                                                             |
| **前置依赖** | marksman 已安装（版本 2026-02-08）                                                                             |
| **完成标准** | 1. 链接补全功能正常<br>2. 定义跳转功能正常<br>3. 引用查找功能正常<br>4. 诊断功能正常（检测断开链接）           |
| **验证方式** | 1. 在编辑器中测试链接补全<br>2. 测试跳转到链接目标<br>3. 测试查找所有引用<br>4. 运行 `marksman check` 检查诊断 |

**验证结果**:

- ✅ 文档符号解析正常
- ✅ LSP 服务器正常
- ❌ 命令行检查不支持（marksman 不支持 `check` 命令）

**结论**: marksman 只能作为 LSP 使用，不能集成到 pre-commit。需要使用 markdownlint-cli2 替代。

---

### BL-502: Markdownlint-cli2 安装与配置

**真实场景**: 项目已有 Gitleaks、Oxlint、Prettier 三个质量门禁，但缺少 Markdown 文档检查。markdownlint-cli2 可以提供命令行 Markdown 检查功能。

**目标**: 安装并配置 markdownlint-cli2

| 项目         | 内容                                                                                                                              |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `package.json`（添加 devDependency）<br>2. `.markdownlint-cli2.jsonc`（新增配置文件）<br>3. `package.json`（添加 npm scripts） |
| **前置依赖** | BL-501 完成                                                                                                                       |
| **完成标准** | 1. markdownlint-cli2 安装成功<br>2. 配置文件创建完成<br>3. npm scripts 添加完成                                                   |
| **验证方式** | 1. `npm install` 成功<br>2. `npx markdownlint-cli2 --version` 显示版本<br>3. `npm run lint:md` 命令可用                           |
...
### BL-503: Markdownlint-cli2 集成到 Pre-commit

**真实场景**: markdownlint-cli2 已安装并配置，需要集成到 pre-commit 质量门禁。

**目标**: 将 markdownlint-cli2 集成到 pre-commit 质量门禁

| 项目         | 内容                                                                                                                      |
| ------------ | ------------------------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `.pre-commit-config.yaml`（修改）                                                                                      |
| **前置依赖** | BL-502 完成                                                                                                               |
| **完成标准** | 1. pre-commit 配置中添加 markdownlint-cli2 hook<br>2. markdownlint-cli2 作为 P3 优先级<br>3. 提交时自动检查 Markdown 文件 |
| **验证方式** | 1. `git commit` 时 markdownlint-cli2 hook 执行<br>2. 有 Markdown 错误时提交失败<br>3. 无错误时提交成功                    |
...
**真实场景**: 运行 markdownlint-cli2 可能发现文档中的格式问题。

**目标**: 修复 markdownlint-cli2 检测到的所有文档问题

| 项目         | 内容                                                                                       |
| ------------ | ------------------------------------------------------------------------------------------ |
| **涉及范围** | 1. `README.md`<br>2. `AGENTS.md`<br>3. `docs/*.md`<br>4. `CHANGELOG.md`<br>5. `BACKLOG.md` |
| **前置依赖** | BL-503 完成                                                                                |
| **完成标准** | 1. `npm run lint:md` 返回 0 错误<br>2. 所有 Markdown 格式正确                              |
| **验证方式** | 1. `npm run lint:md` 无错误<br>2. `git commit` 时 pre-commit 通过                          |
...
### BL-505: Markdownlint 诊断规则配置

**真实场景**: markdownlint-cli2 默认配置可能不适合本项目，需要根据项目需求调整。

**目标**: 配置 markdownlint-cli2 诊断规则

| 项目         | 内容                                                         |
| ------------ | ------------------------------------------------------------ |
| **涉及范围** | 1. `.markdownlint-cli2.jsonc`（修改配置）                    |
| **前置依赖** | BL-502 完成                                                  |
| **完成标准** | 1. 配置文件规则合理<br>2. 忽略规则正确                       |
| **验证方式** | 1. `npm run lint:md` 使用配置文件<br>2. 预期忽略的文件被忽略 |

**配置规则**:

- `MD013`: 禁用行长度检查（允许长行）
- `MD033`: 允许内联 HTML（用于特殊格式）
- `MD041`: 允许首行不是 H1（用于特殊文档）
- `MD024`: 允许重复标题（用于 CHANGELOG）

---

### BL-506: 文档规范制定

**真实场景**: 项目缺少统一的 Markdown 编写规范，导致文档风格不一致。

**目标**: 制定 Markdown 编写规范

| 项目         | 内容                                                                               |
| ------------ | ---------------------------------------------------------------------------------- |
| **涉及范围** | 1. `AGENTS.md`（新增"文档规范"章节）                                               |
| **前置依赖** | BL-504 完成                                                                        |
| **完成标准** | 1. 定义标题层级规范<br>2. 定义链接格式规范<br>3. 定义代码块规范<br>4. 定义表格规范 |
| **验证方式** | 1. AGENTS.md 包含"文档规范"章节<br>2. 规范内容完整、清晰                           |
...
> 完成 v2.9.0 的验证和文档同步工作

### BL-507: Pre-commit hook 验证

**真实场景**: pre-commit 配置已添加 markdownlint-cli2 hook，但未实际测试 hook 执行。需要验证 hook 能正常工作。

**目标**: 验证 pre-commit hook 能正常执行

| 项目         | 内容                                                                                              |
| ------------ | ------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `.pre-commit-config.yaml`<br>2. 测试文件（临时创建）                                           |
| **前置依赖** | BL-503 完成                                                                                       |
| **完成标准** | 1. `git commit` 时 markdownlint-cli2 hook 执行<br>2. 有错误时提交失败<br>3. 无错误时提交成功      |
| **验证方式** | 1. 创建测试文件，故意引入错误<br>2. 运行 `git commit` 验证 hook 执行<br>3. 修复错误后再次提交验证 |

**测试步骤**:

1. 创建临时测试文件，包含 Markdown 错误
2. 运行 `git add` 和 `git commit`
3. 验证 hook 检测到错误并阻止提交
4. 修复错误后再次提交
5. 验证提交成功
...
**真实场景**: v2.9.0 的变更未记录到 BACKLOG.md 和 CHANGELOG.md。需要同步更新文档。

**目标**: 更新 BACKLOG.md 和 CHANGELOG.md

| 项目         | 内容                                                                                                                                      |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| **涉及范围** | 1. `BACKLOG.md`（标记 BL-501~506 为已完成）<br>2. `CHANGELOG.md`（添加 v2.9.0 变更记录）<br>3. `README.md`（添加 lint:md 命令说明）       |
| **前置依赖** | BL-507 完成                                                                                                                               |
| **完成标准** | 1. BACKLOG.md 包含 v2.9.0 已完成任务<br>2. CHANGELOG.md 包含 v2.9.0 变更记录<br>3. README.md 包含 lint:md 命令说明                        |
| **验证方式** | 1. `grep -c "BL-501.*✅" BACKLOG.md` 返回 > 0<br>2. `grep -c "v2.9.0" CHANGELOG.md` 返回 > 0<br>3. `grep -c "lint:md" README.md` 返回 > 0 |
...
**真实场景**: 所有 v2.9.0 的改动都在本地，未持久化到 Git。需要提交并推送。

**目标**: 提交并推送所有 v2.9.0 变更

| 项目         | 内容                                                                        |
| ------------ | --------------------------------------------------------------------------- |
| **涉及范围** | 所有 v2.9.0 修改的文件                                                      |
| **前置依赖** | BL-508 完成                                                                 |
| **完成标准** | 1. 所有变更已提交<br>2. 已推送到远程仓库<br>3. pre-commit hook 通过         |
| **验证方式** | 1. `git status` 显示干净<br>2. `git log` 显示最新提交<br>3. `git push` 成功 |

**提交文件清单**:

- `package.json` - 添加 markdownlint-cli2 依赖和 npm scripts
- `package-lock.json` - 依赖锁定文件
- `.markdownlint-cli2.jsonc` - 新增配置文件
- `.pre-commit-config.yaml` - 添加 markdownlint-cli2 hook
- `AGENTS.md` - 新增"文档规范"章节
- `BACKLOG.md` - 更新任务状态
- `CHANGELOG.md` - 添加变更记录
- 其他修复的 Markdown 文件
