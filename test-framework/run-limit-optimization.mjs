...
async function main() {
  ...
  for (const config of limitConfigs) {
    ...
    console.log(
      `   Semantic: ${config.semantic}, Keyword: ${config.keyword}, Hybrid: ${config.hybrid}`,
    );
  ...
  allResults.forEach((result, i) => {
    ...
    console.log(
      `${prefix} ${result.config.padEnd(15)} ${JSON.stringify(result.limits)}`,
    );
    console.log(`   总体指标:`);
    console.log(`     Recall@10:    ${(result.avgRecall * 100).toFixed(2)}%`);
    console.log(
      `     Precision@10: ${(result.avgPrecision * 100).toFixed(2)}%`,
    );
    console.log(`     MRR:          ${result.avgMRR.toFixed(4)}`);
    ...
    for (const [mode, stats] of Object.entries(result.modeStats)) {
      console.log(
        `     ${mode.padEnd(10)}: Recall ${(stats.recall * 100).toFixed(1)}%, Precision ${(stats.precision * 100).toFixed(1)}%, MRR ${stats.mrr.toFixed(3)}`,
      );
    }
    console.log();
  });
