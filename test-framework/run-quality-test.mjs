...
async function main() {
  ...
  console.log(
    `   ✅ 已加载 ${tools.memoryData.length} 条文档，耗时 ${loadTime}ms\n`,
  );
  ...
  for (let i = 0; i < labeledDataset.queries.length; i++) {
    ...
    for (let j = 0; j < searchResults.length; j++) {
      if (foundIds.has(query.relevant[0])) {
        const firstRelevantIndex = searchResults.findIndex((r) =>
          query.relevant.includes(r.id),
        );
    ...
    if ((i + 1) % 5 === 0 || i === labeledDataset.queries.length - 1) {
      const progress = (
        ((i + 1) / labeledDataset.queries.length) *
        100
      ).toFixed(1);
      const avgRecall =
        results.reduce((sum, r) => sum + r.recall, 0) / results.length;
      console.log(
        `   进度: ${i + 1}/${labeledDataset.queries.length} (${progress}%) - Recall@10: ${(avgRecall * 100).toFixed(1)}%`,
      );
    }
  }
  ...
  console.log("📈 聚合质量指标:");
  console.log(
    `   Recall@10:    ${(avgRecall * 100).toFixed(2)}%  (目标: ≥70%)  ${avgRecall >= 0.7 ? "✅" : "❌"}`,
  );
  console.log(
    `   Precision@10: ${(avgPrecision * 100).toFixed(2)}%  (目标: ≥50%)  ${avgPrecision >= 0.5 ? "✅" : "❌"}`,
  );
  console.log(
    `   MRR:          ${avgMRR.toFixed(4)}   (目标: ≥0.6)  ${avgMRR >= 0.6 ? "✅" : "❌"}`,
  );
  ...
  for (const [mode, stats] of Object.entries(modeGroups)) {
    console.log(
      `   ${mode.padEnd(12)}: Recall ${((stats.recall / stats.count) * 100).toFixed(1)}%, Precision ${((stats.precision / stats.count) * 100).toFixed(1)}%, MRR ${(stats.mrr / stats.count).toFixed(3)}, Queries: ${stats.count}`,
    );
  }
  ...
  results.slice(0, 5).forEach((r, i) => {
    console.log(
      `   ${i + 1}. ${r.query.padEnd(20)}: ${(r.recall * 100).toFixed(1)}% (${r.found}/${r.total})`,
    );
  });
