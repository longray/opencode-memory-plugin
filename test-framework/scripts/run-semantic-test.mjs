...
const CONFIG = {
  embeddingEndpoint:
    process.env.EMBEDDING_ENDPOINT || "http://localhost:18000/v1/embeddings",
...
class MockMemoryTools {
  ...
  async memory_search({ query, mode = "hybrid", limit = 10, threshold = 0.3 }) {
    // 模拟语义搜索
...
class TestExecutionEngine {
  ...
  async runTest(testDataPath, queriesPath) {
    ...
    try {
      ...
      logger.success(
        `Loaded ${testData.memories.length} memories and ${queriesData.queries.length} queries`,
      );
  ...
  async ingestMemories(memories) {
    ...
    logger.success(
      `Memory ingestion: ${successCount} succeeded, ${failCount} failed`,
    );
  }
  ...
  async executeQueries(queries) {
    for (let i = 0; i < queries.length; i++) {
      ...
      try {
        ...
        const searchResult = await this.tools.memory_search({
          query: query.query,
          mode: "hybrid",
          limit: 5,
          threshold: 0.3,
        ...
        if (analysis.passed) {
          logger.success(
            `  ✓ PASS (F1: ${analysis.f1.toFixed(2)}, Sim: ${analysis.top_similarity.toFixed(2)})`,
          );
        } else {
          logger.warn(
            `  ✗ FAIL (F1: ${analysis.f1.toFixed(2)}, Sim: ${analysis.top_similarity.toFixed(2)})`,
          );
        }
      } catch (error) {
  ...
  calculateMetrics() {
    ...
    const positiveResults = results.filter(
      (r) => r.expected_result === "should_recall",
    );
    const negativeResults = results.filter(
      (r) => r.expected_result === "should_not_recall",
    );
    ...
    const f1 =
      precision + recall > 0
        ...
        : 0;
    const accuracy =
      (totalTP + totalTN) / (totalTP + totalFP + totalFN + totalTN) || 0;
    ...
    // 计算Top-K准确率
    const top1Accuracy =
      positiveResults.filter((r) => r.metrics.top1).length /
        positiveResults.length || 0;
    const top3Accuracy =
      positiveResults.filter((r) => r.metrics.top3).length /
        positiveResults.length || 0;
    const top5Accuracy =
      positiveResults.filter((r) => r.metrics.top5).length /
        positiveResults.length || 0;
    ...
    return {
      ...
      by_result_type: {
        positive: {
          ...
          avg_f1:
            positiveResults.reduce((sum, r) => sum + r.metrics.f1, 0) /
              positiveResults.length || 0,
        },
        negative: {
          ...
          avg_f1:
            negativeResults.reduce((sum, r) => sum + r.metrics.f1, 0) /
...
| 召回率 (Recall) | ${(overall_metrics.recall * 100).toFixed(2)}% | > 80% | ${overall_metrics.recall >= 0.8 ? "✅" : "❌"} |
...
| MRR | ${overall_metrics.mrr.toFixed(4)} | > 0.70 | ${overall_metrics.mrr >= 0.7 ? "✅" : "❌"} |

## 排名质量指标

| 指标 | 值 | 目标 | 状态 |
|------|------|------|------|
| Top-1准确率 | ${(ranking_metrics.top1_accuracy * 100).toFixed(2)}% | > 70% | ${ranking_metrics.top1_accuracy >= 0.7 ? "✅" : "❌"} |
| Top-3准确率 | ${(ranking_metrics.top3_accuracy * 100).toFixed(2)}% | > 85% | ${ranking_metrics.top3_accuracy >= 0.85 ? "✅" : "❌"} |
| Top-5准确率 | ${(ranking_metrics.top5_accuracy * 100).toFixed(2)}% | > 90% | ${ranking_metrics.top5_accuracy >= 0.9 ? "✅" : "❌"} |
...
`;
  ...
  printConsoleSummary(metrics) {
    ...
    console.log(
      `  通过: ${summary.passed} ✅ (${(summary.pass_rate * 100).toFixed(2)}%)`,
    );
    ...
    console.log("  关键指标:");
    console.log(
      `    精确率: ${(overall_metrics.precision * 100).toFixed(2)}% ${overall_metrics.precision >= 0.85 ? "✅" : "❌"}`,
    );
    console.log(
      `    召回率: ${(overall_metrics.recall * 100).toFixed(2)}% ${overall_metrics.recall >= 0.8 ? "✅" : "❌"}`,
    );
    console.log(
      `    F1分数: ${overall_metrics.f1.toFixed(4)} ${overall_metrics.f1 >= 0.82 ? "✅" : "❌"}`,
    );
    console.log(
      `    Top-1:  ${(ranking_metrics.top1_accuracy * 100).toFixed(2)}% ${ranking_metrics.top1_accuracy >= 0.7 ? "✅" : "❌"}`,
    );
    console.log("");
    console.log("=".repeat(80));
    console.log("");
...
Environment Variables:
  ...
  const testDataPath =
    dataIndex !== -1
      ? args[dataIndex + 1]
      : path.join(CONFIG.testDataDir, "retrieval-memories.json");
  const queriesPath =
    queriesIndex !== -1
      ? args[queriesIndex + 1]
      : path.join(CONFIG.testDataDir, "retrieval-queries.json");
  const outputDir =
    outputIndex !== -1 ? args[outputIndex + 1] : CONFIG.outputDir;
  ...
  try {
    ...
    const logPath = path.join(
      CONFIG.outputDir,
      `test-execution-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
    );
