...
async function testRealModeDetailed() {
  ...
  try {
    ...
    if (bm25Results.length > 0) {
      console.log(`   第1个结果: ${bm25Results[0].content.substring(0, 50)}...`);
    }
    ...
    if (vectorResults.length > 0) {
      console.log(`   第1个结果: ${vectorResults[0].content.substring(0, 50)}...`);
    }
    ...
    if (hybridResults.length > 0) {
      console.log(`   第1个结果: ${hybridResults[0].content.substring(0, 50)}...`);
    }
    ...
    console.log(`✅ vector_memory_search(hybrid): ${searchResults.length}条`);
    ...
    if (keywordResults.length > 0) {
      console.log(`   第1个结果: ${keywordResults[0].content.substring(0, 50)}...`);
    }
    ...
    console.log(`✅ 工具搜索: ${searchResults.length}条 + ${keywordResults.length}条结果`);
    console.log('\n✅ Real模式验证通过！');
    console.log('='.repeat(60));
