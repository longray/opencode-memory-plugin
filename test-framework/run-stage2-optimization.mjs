...
class Stage2OptimizedTools {
  constructor(options = {}) {
    ...
    // 阶段2优化选项
    this.enableMultiplicativeFusion = options.enableMultiplicativeFusion ?? true;
  ...
  hybridSearchMultiplicative(vectorScores, bm25Scores) {
    ...
    for (const record of this.memoryData) {
      ...
      // 乘法融合
      const hybridScore = Math.pow(normalizedVec, 0.7) * Math.pow(normalizedBM25, 0.3);
  ...
  hybridSearchDynamicWeight(vectorScores, bm25Scores) {
    ...
    for (const record of this.memoryData) {
      ...
      const hybridScore = vectorWeight * normalizedVec + bm25Weight * normalizedBM25;
  ...
  async vector_memory_search({ query, mode = 'hybrid', limit = 10 }) {
    ...
    // 查询扩展
    const expandedQueries = this.enableQueryExpansion ? expandQuery(query) : [query];
...
async function main() {
  ...
  for (const config of testConfigs) {
    ...
    for (const [mode, stats] of Object.entries(modeStats)) {
      console.log(`     ${mode.padEnd(10)}: Recall ${(stats.recall * 100).toFixed(1)}%, Precision ${(stats.precision * 100).toFixed(1)}%`);
    }
  }
  ...
  console.log(`${'配置'.padEnd(25)} | ${'Recall@10'.padStart(12)} | ${'Precision@10'.padStart(12)} | ${'MRR'.padStart(10)}`);
  console.log('-'.repeat(60));
  
  sortedByPrecision.forEach((result, i) => {
    const prefix = i === 0 ? '🏆' : '  ';
    console.log(`${prefix} ${result.config.padEnd(25)} | ${(result.avgRecall * 100).toFixed(2).padStart(11)}% | ${(result.avgPrecision * 100).toFixed(2).padStart(11)}% | ${result.avgMRR.toFixed(4).padStart(10)}`);
  });
