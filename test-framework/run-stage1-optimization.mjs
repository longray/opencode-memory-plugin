...
async function main() {
  ...
  for (const query of queries) {
    ...
    if (results.length % 5 === 0 || results.length === queries.length) {
      ...
      console.log(
        `   进度: ${results.length}/${queries.length} - Recall: ${(avgRecall * 100).toFixed(1)}%`,
      );
    }
  }
  ...
  for (const [mode, stats] of Object.entries(modeStats)) {
    ...
    console.log(
      `   ${mode.padEnd(10)}: Recall ${(stats.recall * 100).toFixed(1)}%, Precision ${(stats.precision * 100).toFixed(1)}%, MRR ${stats.mrr.toFixed(3)}, Queries: ${stats.count}`,
    );
  }
