...
async function compareModes(mockResult, realResult) {
  ...
  console.log(`|------|---------|----------|------|`);
  console.log(
    `| 向量生成耗时 | ${mockResult.avgEmbeddingTime}ms | ${realResult.avgEmbeddingTime}ms | ${realResult.avgEmbeddingTime - mockResult.avgEmbeddingTime}ms |`,
  );
  console.log(
    `| 向量维度 | ${mockResult.vectorDimensions} | ${realResult.vectorDimensions} | ${realResult.vectorDimensions - mockResult.vectorDimensions} |`,
  );
  console.log(
    `| 搜索结果数 | ${mockResult.searchResults} | ${realResult.searchResults} | ${realResult.searchResults - mockResult.searchResults} |`,
  );
  ...
  console.log(
    `  维度匹配: ${mockResult.vectorDimensions === realResult.vectorDimensions ? "❌" : "✅"} (${mockResult.vectorDimensions} vs ${realResult.vectorDimensions})`,
  );
  ...
  const report = {
    ...
    comparison: {
      ...
      dimensionMismatch:
        mockResult.vectorDimensions !== realResult.vectorDimensions,
      dimensionDifference:
        realResult.vectorDimensions - mockResult.vectorDimensions,
    },
  };
