...
async function sampleRealAPITest() {
  ...
  console.log(`   平均耗时: ${(writeDuration / writeResults.length).toFixed(2)}ms`);
  ...
  const avgEmbedTime = embedTimes.reduce((sum, t) => sum + t, 0) / embedTimes.length;
  const minEmbedTime = Math.min(...embedTimes);
  const maxEmbedTime = Math.max(...embedTimes);
  const p95EmbedTime = embedTimes.sort((a, b) => a - b)[Math.floor(embedTimes.length * 0.95)];
  ...
  console.log(`   平均耗时: ${(batchDuration / batchResults.length).toFixed(2)}ms`);
  console.log(`   吞吐量: ${(batchResults.length / batchDuration * 1000).toFixed(2)} embeddings/秒`);
  ...
  for (const test of searchTests) {
    const startTime = Date.now();
    const results = await tools.vector_memory_search(test);
    const duration = Date.now() - startTime;
    searchTimes.push({ ...test, duration, resultCount: results.length });
    console.log(`  ${test.mode}搜索 "${test.query}": ${duration}ms (${results.length}条结果)`);
  }

  const avgSearchTime = searchTimes.reduce((sum, t) => sum + t.duration, 0) / searchTimes.length;
  const avgSearchResults = searchTimes.reduce((sum, t) => sum + t.resultCount, 0) / searchTimes.length;
  ...
  console.log('\n测试5: 缓存效果\n');
  const cacheHitTests = [
    '开发测试', '开发测试', '开发测试', '开发测试', '开发测试',
    '数据库测试', '数据库测试', '数据库测试', '数据库测试', '数据库测试',
  ];
  ...
  const avgCacheHitTime = cacheHitTimes.reduce((sum, t) => sum + t, 0) / cacheHitTimes.length;
  ...
  const reportPath = path.join(__dirname, '..', 'test-results', 'real-api-performance-report.json');
  ...
  console.log(`单个Embedding: ${report.tests.singleEmbedding.avgTime}ms (P95: ${report.tests.singleEmbedding.p95Time}ms)`);
