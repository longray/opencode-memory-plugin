...
async function testWeightConfig(config, tools, hybridQueries) {
  const originalVectorMemorySearch = tools.memory_search.bind(tools);

  tools.memory_search = async function ({ query, mode, limit = 10 }) {
    if (mode !== "hybrid") {
    ...
    const results = records
      .map((record) => {
        const normalizedVector =
          (vectorScores.get(record.id) || 0) / maxVectorScore;
        const normalizedBM25 = (bm25Scores.get(record.id) || 0) / maxBM25Score;
        const hybridScore =
          config.vector * normalizedVector + config.bm25 * normalizedBM25;
  ...
  tools.memory_search = originalVectorMemorySearch;
...
async function main() {
  ...
  const hybridQueries = labeledDataset.queries.filter(
    (q) => q.mode === "hybrid",
  );
  ...
  allResults.forEach((result, i) => {
    ...
    if (result.avgRecall + result.avgPrecision > 0) {
      const f1 =
        (2 * result.avgRecall * result.avgPrecision) /
