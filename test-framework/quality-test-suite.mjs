...
export function createQualityTestSuite() {
  ...
  for (const queryData of queries) {
    testCases.push({
      ...
      execute: async (engine) => {
        ...
        // 使用 memory_search 进行搜索
        const results = await engine.options.tools.memory_search({
          query: queryData.query,
          mode: queryData.mode,
          limit: 10,
        });
  ...
  testCases.push({
    ...
    execute: async (engine) => {
      ...
      const avgMRR =
        allMetrics.reduce((sum, m) => sum + m.mrr, 0) / allMetrics.length;
      const avgNDCG =
        allMetrics.reduce((sum, m) => sum + m.ndcg, 0) / allMetrics.length;
      const avgF1 =
      ...
      for (const [mode, data] of Object.entries(byMode)) {
        modeStats[mode] = {
          ...
          avgPrecision: (
            data.precisions.reduce((a, b) => a + b, 0) / data.precisions.length
          ).toFixed(4),
          avgMRR: (
            data.mrrs.reduce((a, b) => a + b, 0) / data.mrrs.length
          ).toFixed(4),
      ...
      const passed =
        avgRecall >= qualityTargets.recallAt10 &&
...
export function generateQualityReport(results) {
  const qualityResults = results.filter(
    (r) => r.testCase.category === "搜索质量",
  );
  const aggregateResult = qualityResults.find(
    (r) => r.testCase.subcategory === "aggregate",
  );
  ...
  if (aggregateResult && aggregateResult.result.aggregateMetrics) {
    ...
    if (aggregateResult.result.modeStats) {
      ...
      for (const [mode, stats] of Object.entries(
        aggregateResult.result.modeStats,
      )) {
  ...
  // 详细结果（按指标排序）
  const detailResults = qualityResults.filter(
    (r) => r.testCase.subcategory !== "aggregate",
  );
  const sortedByRecall = [...detailResults].sort(
    (a, b) =>
      parseFloat(b.result.metrics?.recall || 0) -
      parseFloat(a.result.metrics?.recall || 0),
  );
  ...
  for (const result of sortedByRecall.slice(0, 20)) {
    // 只显示前20条
